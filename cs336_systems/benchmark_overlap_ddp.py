...
parser.add_argument("--use_nsight", action="store_true")
...
def ddp_demo(rank, world_size, args):
    ...
    overlap_ddp = OverlapDDP(model) # Wrap-up function, make it ready for communication
    ...
    for _ in range(args.num_iters):
        ...
        overlap_ddp.finish_grad_synchronization()
    ...
    if rank == 1:
        print(f"Total time for iterations is {((end - start) * 1000):.2f} ms. Total time for communication is {((overlap_ddp.get_comm_time()) * 1000):.2f} ms.")
